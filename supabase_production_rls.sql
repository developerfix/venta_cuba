...
-- This will resolve ALL 6 RLS warnings in your Supabase dashboard
...
DROP POLICY IF EXISTS "Allow all operations" ON public.device_tokens;
...
CREATE POLICY "secure_tokens_access" ON public.device_tokens
  FOR ALL USING (
    user_id = current_setting('app.current_user_id', true) OR
    user_id IS NULL  -- Allow anonymous tokens for initial setup
  )
  WITH CHECK (
    user_id = current_setting('app.current_user_id', true) OR
    user_id IS NULL  -- Allow anonymous tokens for initial setup
